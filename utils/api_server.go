...
func (api *TrendAPI) handleTrendAll(w http.ResponseWriter, r *http.Request) {
	...
	if btcResult, ok := api.latestResults[btcKey]; ok {
		apiStatus := "unknown"
		if btcResult.Status == UpTrend {
			apiStatus = "up"
		} else if btcResult.Status == DownTrend {
			apiStatus = "down"
		} else {
			apiStatus = "range"
		}
		...
		response["btc"] = map[string]interface{}{
			...
			"ema60":         btcResult.EMA60,
			"ema120":        btcResult.EMA120,
	...
	if ethResult, ok := api.latestResults[ethKey]; ok {
		apiStatus := "unknown"
		if ethResult.Status == UpTrend {
			apiStatus = "up"
		} else if ethResult.Status == DownTrend {
			apiStatus = "down"
		} else {
			apiStatus = "range"
		}
		response["eth"] = map[string]interface{}{
			"symbol":        "ETH",
			"interval":      ethResult.Interval,
			"trend":         apiStatus,
			"current_price": ethResult.CurrentPrice,
			"ema25":         ethResult.EMA25,
			"ema60":         ethResult.EMA60,
			"ema120":        ethResult.EMA120,
			"time":          ethResult.Time.Format("2006-01-02 15:04:05"),
		}
	} else {
		response["eth"] = map[string]string{
			"error": "unknown",
		}
	}
	// 返回JSON响应
